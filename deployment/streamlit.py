...
def display_batch(images, pred_labels, labels):
    ...
    for j, (image, pred_class, label) in enumerate(zip(images, pred_labels, labels)):
        ...
        cols[j%4].image(image_numpy, clamp = True, caption=f"Pred: {pred_class.item()}, Actual: {label.item()}", use_column_width=True)
...
images, pred_labels, labels = process_data(model, batch)
tp_count, fp_count, fn_count, tn_count = calculate_stats(pred_labels, labels)
st.write(f"True Positives: {tp_count}, True Negatives: {tn_count}")
st.write(f"False Positives: {fp_count}, False Negatives: {fn_count}")
